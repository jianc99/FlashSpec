...
from FlashSpec.Engine.backend_snapspec import LMBackend
...
parser.add_argument('--snap_budget', type=int, default=256, help='SnapKV cache budget')
parser.add_argument('--window_size', type=int, default=32, help='Window size for SnapKV')
parser.add_argument('--kernel_size', type=int, default=5, help='Kernel size for SnapKV')
...
engine.setup_caches(max_batch_size=BATCH_SIZE, max_seq_length=MAX_LEN_TARGET, 
                    snap_budget=args.snap_budget, window_size=args.window_size, kernel_size=args.kernel_size, pooling_type="avg")
...
# load dataset
# pg-19 dataset. TODO: add RULER dataset, InfiniBench dataset
repeats = 20
...
for step, batch in tqdm(enumerate(dataloader), total=num_eval_steps):
    ...
    while terminal == False:
        ...
        prespec_cachelens = engine.cachelens.clone()
        # Draft speculation
        if benchmark:
            torch.cuda.synchronize()
            t1 = time.time()
        ...
        engine.cachelens = prespec_cachelens
    ...
    if args.printoutput:
        for i in range(BATCH_SIZE):
            print(colored(tokenizer.decode(input_ids[i, -20:]), "red"))
            print(colored(tokenizer.decode(output[i, args.prefix_len:num_nodes[i]])), "green")
