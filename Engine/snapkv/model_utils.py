...
def snap_attn_forward(self,
        ...
        if past_key_value is not None:
            ...
            if not self.is_snap or is_decode:
                key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            else:
                # only to be used for the last prefill set of tokens
                output = past_key_value.snap_update(key_states, value_states, self.layer_idx, query_states, cache_kwargs)
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, LlamaFlashAttention2):
            model._modules[name].forward = types.MethodType(
                snap_attn_forward, model._modules[name]
            )
            model._modules[name].is_snap = True

def disable_snap(model):
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            disable_snap(
                module,
        ...
        if isinstance(module, LlamaFlashAttention2):
            model._modules[name].is_snap = False